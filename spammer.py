...
for _ in range(times):
	...
	pyautogui.moveTo(800,900) # moves the cursor to the parameters which are the respective coordinates.
	pyautogui.click() # clicks the position at which the cursor is.
	pyautogui.hotkey('ctrl','v') # presses (ctrl + v) which is the shortcut button for paste.
	pyautogui.hotkey('enter') # presses the enter button which sends the spam message for a single time.
	pyautogui.click() # again click the position at which the cursor is, i.e. the text box on the chat window.
